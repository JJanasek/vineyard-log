...
- ~~**Disease risk flags from the weather data**~~ – done: the Open-Meteo fetch also stores per-day leaf-wetness hours and warm/hot hours; the Weather tab shows peronospora (3-10 primary rule, wet warm nights), an oidium index (Gubler-Thomas style) and a botrytis wet-period indicator with a 3-day outlook, and the Vineyard tab a compact version. Weather-only indicators, labelled as such.
...
- **ČHMÚ station data** – second weather source from the Czech open-data portal (rain gauge 4 km from the vineyard beats a model); measured values override Open-Meteo, typed days stay.
- **Prefill from planted varieties** – Czech variety catalogue drives block defaults, season plan items, harvest targets, risk emphasis and cellar protocol templates.
- **Reminders and an overview tab** – user-defined recurring reminders (e.g. weekly sugar sampling from veraison) and automatic ones (PHI end, plan tasks, high disease risk, stuck fermentation) as Android notifications; an overview tab with risk, reminders and the season charts.
- **Photo intelligence** – guide topics can take your own photos first (labelled gallery), later a small on-device classifier for the common diseases and deficiencies.
