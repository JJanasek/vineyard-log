...
- **Overview tab and more automatic reminders** – reminders themselves are done (above). Open: an *Overview* tab that puts risk, upcoming reminders and open plan tasks above the season charts (season weather with sprays and risk, GDD across vintages, ripening per block, harvest per year); automatic reminders for plan tasks at the start of their window, high disease risk after a background weather fetch (WorkManager), stuck fermentation and the next sampling date from the harvest estimate.
...
- **Cellar and spray templates** – protocol skeletons (aromatic white without MLF, rosé from Modrý Portugal, red) that create the batch's planned steps, and a spray-programme skeleton by phenology stage with the products you own.
