...
UI = {
 'en': dict(home='Home', menu='Menu', sources='Sources', sources_for='Sources for this page', typical_nm='Typical °NM', grades_title='Must sugar categories of the Czech wine law', grades_note='Thresholds from Act No. 321/2004 Coll., §§ 17–19; a degree of the normalised must meter is kilograms of fermentable sugar per hectolitre of must. The column above is a typical picking sugar for a dry wine, not a rule – in the app the target is set per block or in Settings.', grade='Category', min_nm='At least °NM', sources_intro='Where the content of this site and the app comes from. Typical values (harvest windows, sugar targets, doses in the templates) are defaults to adjust for your site; the product label and the lab result always win.', book='book', guide='Field guide', phenology='Growth stages', varieties='Varieties', spray='Spray programme', cellar='Cellar protocols', docs='Docs', viewer='Backup viewer', download='Download', switch='Česky',
    ...
    varieties_intro='Varieties grown in Moravia with the app\'s built-in values: colour, ripening class, usual harvest window, a typical picking sugar and disease sensitivity (L/M/H). The sugar column is the range the variety\'s Czech Wikipedia article gives and is blank where that article states none; there is no official per-variety dataset, since grapevine is exempt from the ÚKZÚS utility-value trials and measured sugar swings by several °NM between sites and years. In the app the sugar you actually pick at is set per block or in Settings.', variety='Variety', style='Colour', ripening='Ripening', harvest_window='Harvest (MM-dd)',
 ...
 'cs': dict(home='Úvod', menu='Menu', sources='Zdroje', sources_for='Zdroje k této stránce', typical_nm='Typicky °NM', grades_title='Kategorie cukernatosti podle vinařského zákona', grades_note='Hranice ze zákona č. 321/2004 Sb., § 17–19; stupeň normalizovaného moštoměru je kilogram zkvasitelných cukrů na hektolitr moštu. Sloupec výše je typická cukernatost sklizně pro suché víno, ne pravidlo – v aplikaci se cíl nastavuje u tratě nebo v Nastavení.', grade='Kategorie', min_nm='Nejméně °NM', sources_intro='Odkud pochází obsah tohoto webu a aplikace. Typické hodnoty (okna sklizně, cílová cukernatost, dávky v šablonách) jsou výchozí hodnoty k úpravě podle stanoviště; etiketa přípravku a laboratorní výsledek mají vždy přednost.', book='kniha', guide='Atlas', phenology='Fenofáze', varieties='Odrůdy', spray='Postřikový program', cellar='Sklepní protokoly', docs='Dokumentace', viewer='Prohlížeč zálohy', download='Stažení', switch='English',
    ...
    varieties_intro='Odrůdy pěstované na Moravě s vestavěnými hodnotami aplikace: barva, ranost, obvyklé okno sklizně, typická cukernatost sklizně a citlivost na choroby (N/S/V). Sloupec cukernatosti je rozsah, který uvádí heslo odrůdy na české Wikipedii, a u odrůd bez údaje zůstává prázdný; oficiální data po odrůdách neexistují, réva je vyňatá ze zkoušek užitné hodnoty ÚKZÚZ a naměřená cukernatost kolísá o několik °NM mezi tratěmi a ročníky. Cukernatost, při které opravdu sklízíš, se v aplikaci nastavuje u tratě nebo v Nastavení.', variety='Odrůda', style='Barva', ripening='Ranost', harvest_window='Sklizeň (MM-dd)',
...
def nm(x):
    """16.0 -> '16', 16.5 -> '16,5' (the site is read in Czech and English alike)."""
...
for lang in ('en', 'cs'):
    ...
    render('varieties.html', 'varieties.html', 1, page='varieties', title=t['varieties'], varieties=varieties, styles=styles, lv=t['levels'], grades=grades, topic=sources_by_key['varieties'])
