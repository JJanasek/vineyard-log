...
let lang = ['cs', 'en'].includes(params.get('lang')) ? params.get('lang') : ['cs', 'en'].includes(stored) ? stored : ((navigator.language || 'cs').toLowerCase().startsWith('cs') ? 'cs' : 'en');
let T = I18N[lang];
let labels = { en: {}, cs: {} };
let data = null;
let tab = 'overview';
...
function applyLang() { $('lbl-open').textContent = T.open; $('btn-download').textContent = T.download; $('btn-lang').textContent = lang === 'cs' ? 'EN' : 'CS'; $('title').textContent = T.appTitle; $('w-title').textContent = T.wTitle; $('w-text').textContent = T.wText; $('w-drop').textContent = T.drop; document.documentElement.lang = lang; }
